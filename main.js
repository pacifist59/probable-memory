...
// [Fix] Handle Like/Interest toggle with reliable cancellation
window.handleLike = async (id) => { 
    ...
    if (existing) {
        ...
        if (error) window.showToast('취소 중 오류가 발생했습니다.', 'error');
        else {
            window.showToast('기록이 안전하게 취소되었습니다.');
            renderSetlistDetailPage(id);
        }
    } else {
        const { error } = await sb.from('likes').insert({ setlist_id: id, user_id: session.user.id }); 
        if (error) window.showToast('저장 중 오류가 발생했습니다.', 'error');
        else {
            window.showToast('당신의 소중한 기록을 저장했습니다!');
            renderSetlistDetailPage(id); 
        }
    }
}
...
window.handleDelete = async (id) => {
    ...
    if (error) window.showToast('삭제 중 오류가 발생했습니다.', 'error');
        ...
        window.showToast('선곡표가 영구적으로 삭제되었습니다.');
...
async function renderSetlistDetailPage(id) {
    ...
    const likeBtnText = isFuture ? '꼭 가고 싶어요!' : '이 공연의 추억을 간직하고 있어요';
    ...
    document.getElementById('page-router').innerHTML = `
        <div id="detail-view" class="animate-fade-in">
            ...
            <div class="lg:hidden fixed bottom-6 left-6 right-6 z-[100] flex gap-3">
                <button onclick="window.handleLike('${data.id}')" class="flex-1 py-5 ${isFuture ? 'bg-indigo-600' : 'bg-pink-500'} text-white rounded-[2rem] font-black shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-all">
                    <i class="fas ${isFuture ? 'fa-star' : 'fa-heart'}"></i> ${isFuture ? '가고 싶어요' : '관람 완료'} <span>${likeCount}</span>
                </button>
...
async function renderDetailView(data, likeCount, comments, targetId, isFuture) {
    ...
    // [Auto] Generate Ticket Search Link
    const ticketSearchQuery = encodeURIComponent(`${data.artist} ${data.concert} 티켓 예매`);
    const autoTicketLink = `https://search.naver.com/search.naver?query=${ticketSearchQuery}`;
        ...
        <div class="animate-fade-in">
            <div class="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
                ...
                <div class="flex gap-3">
                    <button onclick="window.handleLike('${data.id}')" class="hidden lg:flex px-8 py-4 ${isFuture ? 'bg-indigo-600' : 'bg-pink-50 dark:bg-pink-950/20'} ${isFuture ? 'text-white' : 'text-pink-500'} rounded-2xl font-black shadow-sm active:scale-95 transition-all items-center gap-3">
                        <i class="fas ${isFuture ? 'fa-star' : 'fa-heart'}"></i>
                        <span>${isFuture ? '꼭 가고 싶어요!' : '이 공연의 추억을 간직하고 있어요'}</span>
                        <span class="ml-2 px-3 py-1 bg-white/20 dark:bg-black/20 rounded-full text-xs">${likeCount}</span>
                    </button>
                    ...
                    <button onclick="window.shareSetlist('${data.id}', '${data.artist}')" class="p-4 bg-gray-100 dark:bg-gray-800 rounded-2xl hover:text-indigo-600 transition-all"><i class="fas fa-share-alt"></i></button>
                </div>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div class="lg:col-span-8 bg-white dark:bg-gray-900 p-10 rounded-[3rem] border dark:border-gray-800 shadow-sm">
                    ${isFuture ? `<a href="${data.ticket_url || autoTicketLink}" target="_blank" class="block w-full py-5 bg-yellow-400 text-yellow-900 text-center rounded-2xl font-black mb-10 hover:bg-yellow-500 transition-all shadow-xl shadow-yellow-400/20 text-xl"><i class="fas fa-ticket-alt mr-2"></i> 티켓 예매 정보 확인하기</a>` : ''}
...
window.postComment = async (id) => {
    ...
    if (error) {
        ...
        window.showToast('당신의 소중한 후기가 등록되었습니다!');
        input.value = ''; 
        renderSetlistDetailPage(id);
    }
}
    ...
    list.innerHTML = data.map(item => {
        ...
        <div onclick="window.navigateTo('setlist', '${item.id}')" class="bg-white dark:bg-gray-900 p-6 sm:p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer flex flex-col sm:flex-row justify-between sm:items-center gap-6 group animate-fade-in">
            ...
            <div class="sm:text-right flex flex-col sm:items-end gap-1">
                ...
                <p class="text-xs font-black text-gray-300 dark:text-gray-600 uppercase tracking-[0.2em]">${isUpcoming ? '보고 싶어요!' : (item.location || 'Global')}</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('setlist-form')?.addEventListener('submit', async (e) => {
        ...
        if (resErr) {
            ...
            window.showToast(editingId ? '선곡표가 성공적으로 수정되었습니다.' : '새로운 선곡표가 등록되었습니다!');
            window.toggleModal(false); 
